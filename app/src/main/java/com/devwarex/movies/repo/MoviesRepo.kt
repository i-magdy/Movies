    ...
    fun getMoviesResult(genreId: Int): Flow<PagingData<Movie>> {
       return if (genreId == -1){
           Pager(
               ...
               pagingSourceFactory = { MovieSourceRepo.getPopularMovieSource(service = service, apikey = apiKey, lang = lang) }
           ).flow
       } else{
           Pager(
               config = PagingConfig(pageSize = 20, enablePlaceholders = false),
               pagingSourceFactory = { MovieSourceRepo.getGenreMoviesSource(genreId = genreId,service = service, apikey = apiKey,lang = lang) }
           ).flow
       }
    }
}